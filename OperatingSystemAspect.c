...
# 7 "OperatingSystem.h" 2
# 37 "OperatingSystem.h" 
enum ProcessStates {NEW,READY,EXECUTING,BLOCKED,EXIT}; 
#line 40 "OperatingSystem.h"
enum SystemCallIdentifiers {SYSCALL_END=3,SYSCALL_YIELD=4,SYSCALL_PRINTEXECPID=5,SYSCALL_PRINTPROCREG=6,SYSCALL_SLEEP=7}; 
#line 55 "OperatingSystem.h"
typedef struct {int busy; int initialPhysicalAddress; int processSize; int state; int priority; int copyOfPCRegister; int copyOfRegisters[10]; unsigned int copyOfPSWRegister; int programListIndex; int queueID; int whenToWakeUp; 
}PCB; 
...
# 795 "OperatingSystemAspect.c"
retValue_acc = __builtin_bswap32(__bsx);
# 797 "OperatingSystemAspect.c"
...
void OperatingSystem_WakeUpProcess(int ); 
...
case (-1): ComputerSystem_DebugMessage(104, 'e', (programList[i]->executableName), "it does not exist"); 
...
default: ComputerSystem_DebugMessage(104, 'e', (programList[i]->executableName), "unknown error"); } } } }else{
{ 
...
ComputerSystem_DebugMessage(111, 'p', PID); 
...
for(i = 0;i < 10;i++) { processTable[PID].copyOfRegisters[i] = 0; } 
...
ComputerSystem_DebugMessage(110, 'p', PID, statesNames[lastState], statesNames[READY]); } }
...
if ((selectedProcess = Heap_poll(readyToRunQueue[i], 1, (&numberOfReadyToRunProcesses[i]))) >= 0){
...
ComputerSystem_DebugMessage(110, 'p', PID, statesNames[lastState], statesNames[EXECUTING]); 
...
for(i = 0;i < 10;i++) { Processor_SetRegister(i, processTable[PID].copyOfRegisters[i]); } 
...
for(i = 0;i < 10;i++) { processTable[PID].copyOfRegisters[i] = Processor_GetRegister(i); } 
...
ComputerSystem_DebugMessage(110, 'p', executingProcessID, statesNames[lastState], statesNames[EXIT]); 
...
if ((selectedProcess = OperatingSystem_ShortTermScheduler()) == (-1)){
...
systemCallID = Processor_GetRegister(1); 
...
case SYSCALL_PRINTPROCREG: OperatingSystem_ShowTime('p'); 
#line 417 "OperatingSystem.c"
ComputerSystem_DebugMessage(130, 'p'); 
#line 418 "OperatingSystem.c"
for(reg = 0;reg < 10;reg++) { { 
#line 419 "OperatingSystem.c"
ComputerSystem_DebugMessage(131, 'p', reg, Processor_GetRegister(reg)); } } 
#line 421 "OperatingSystem.c"
break; 
#line 422 "OperatingSystem.c"
case SYSCALL_YIELD: if ((newPID = OperatingSystem_GetProcessWithSamePriorityToRun(executingProcessID)) != (-1)){
{ 
#line 424 "OperatingSystem.c"
oldPID = executingProcessID; 
#line 425 "OperatingSystem.c"
OperatingSystem_ExtractFromReadyToRun(processTable[oldPID].queueID); 
...
ComputerSystem_DebugMessage(115, 'p', oldPID, executingProcessID); 
...
case SYSCALL_SLEEP: OperatingSystem_UpdateWhenToWakeUp(executingProcessID); 
...
if ((newPID = OperatingSystem_ShortTermScheduler()) == (-1)){
newPID = sipID; }
...
OperatingSystem_WakeUpProcess(pid); 
...
void OperatingSystem_WakeUpProcess(int selectedProcess)  
...
if (processTable[executingProcessID].priority <= processTable[selectedProcess].priority){
{ 
#line 483 "OperatingSystem.c"
OperatingSystem_MoveToTheREADYState(selectedProcess); } }else{
{ 
#line 486 "OperatingSystem.c"
OperatingSystem_ShowTime('s'); 
#line 487 "OperatingSystem.c"
ComputerSystem_DebugMessage(121, 's', executingProcessID, selectedProcess); 
...
case CLOCK_BIT: OperatingSystem_HandleClockInterrupt(); 
...
ComputerSystem_DebugMessage(112, 's', queuesNames[queueID]); 
...
ComputerSystem_DebugMessage(107, 's', id, processTable[id].priority); 
...
ComputerSystem_DebugMessage(110, 'p', pid, statesNames[lastState], statesNames[BLOCKED]); 
...
retValue_acc = Heap_getFirst(sleepingProcessesQueue, numberOfSleepingProcesses);
...
retValue_acc = Heap_poll(sleepingProcessesQueue, 0, (&numberOfSleepingProcesses));
