 ...
 for (i=0;programList[i] != ((void *)0) && i < 20 ; i++) {
  if ((PID = OperatingSystem_CreateProcess(i)) < 0) {
   ...
   case -4:
    ...
    break;
   default:
    ComputerSystem_DebugMessage(104,'e',programList[i]->executableName, "unknown error");
   }
  }
  else {
...
void OperatingSystem_PCBInitialization(int PID, int initialPhysicalAddress, int processSize, int priority, int processPLIndex) {
 ...
 int i;
 for (i = 0;i < 10;i++)
  processTable[PID].copyOfRegisters[i] = 0;
}
...
void OperatingSystem_HandleSystemCall() {
 ...
 switch (systemCallID) {
  ...
  case SYSCALL_PRINTPROCREG:
   ...
   break;
  case SYSCALL_YIELD:
   if ((newPID = OperatingSystem_GetProcessWithSamePriorityToRun(executingProcessID)) != -1) {
...
int OperatingSystem_ExtractFromSleepingProcessesQueue() {
 return Heap_poll(sleepingProcessesQueue, 0, &numberOfSleepingProcesses);
}
